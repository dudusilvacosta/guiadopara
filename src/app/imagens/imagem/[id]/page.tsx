import { getImagemPorId } from "@/data/imagens";
...
export default async function ImagemPage({ params }: Props) {
  ...
  const imagem = getImagemPorId(Number(id));

  if (!imagem) {
    return <p>Imagem não encontrada.</p>;
  }

  const lat = Number(imagem.coords?.[0]);
  const lng = Number(imagem.coords?.[1]);
    ...
    <div style={{ margin: "0 1rem" }}>
      ...
      {isValid && (
        <a href={mapsUrl!} target="_blank" rel="noopener noreferrer">
          <button className={style.botaoMaps}>
